...
function App() {
  ...
  const [titleApp, setTitleApp] = useState('¿Quieres ser mi novia?');
  ...
  const handleClickYes = () => {
    setAccept(true);
    setTitleApp('Sabía que dirias que si. :*');

    setTimeout(() => {
      setAccept(false);
      setTitleApp('¿Quieres ser mi novia?');
    }, 20000);
  };
  ...
  return (
    <main className="container">
      ...
      <div className="app">
        ...
        />
        <Button onclick={handleClickYes} text="Sí" classes="btn_margin_yes" />
      </div>
      {accept
